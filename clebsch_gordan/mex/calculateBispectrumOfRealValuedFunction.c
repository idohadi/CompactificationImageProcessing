 ...
 * 
 * MATLAB call form:
 *      [bispectrum, bispectrum_gradient] = calculateBispectrumOfRealValuedFunction(shc, bandlimit)
 *  where
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    // Create output
    plhs[0] = mxCreateDoubleMatrix(bispectrum_lookup_table[bandlimit][bandlimit][bandlimit]+1, 1, mxREAL);
    if (nlhs>1)
    {
        plhs[1] = mxCreateDoubleMatrix((bandlimit+1)*(bandlimit+1), bispectrum_lookup_table[bandlimit][bandlimit][bandlimit]+1, mxREAL);
    }
    
    // Calculate the bispectrm / its gradient
    r_calculate_bispectrum(shc, bandlimit, bispectrum_lookup_table, cb_coeffs, mxGetDoubles(plhs[0]));
    if (nlhs>1)
    {
        r_calculate_bispectrum_gradient(shc, bandlimit, bispectrum_lookup_table, cb_coeffs, mxGetDoubles(plhs[1]));
    }
}