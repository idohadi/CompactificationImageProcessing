...
% The changes:
%   (1) I removed all the data generating code and added the data as an
%       input argument.
%   (2) I parameter coeff was not used. All references to it were removed.
% Removal was done by commenting out the code.
% 
% ***********************************************************
% Author    Ido Hadi
...
%         goodshift=0;
%         while goodshift==0
%             shift=randi([-shift_range,shift_range],1,2);
%             if norm(shift)<=shift_range
%                 goodshift=1;
%             end
%         end
%         rotdata=imrotate(D(:,:,I),theta,'nearest','crop');
%         data(:,:,i+(I-1)*Ncopy)=rotdata(shift_range+1+shift(1):shift_range+sidel+shift(1),shift_range+1+shift(2):shift_range+sidel+shift(2));
%     end
% end
% clear D
% 
